...
//! let (removed, hot_events) = index.invalidate_and_get_events(
//!     &maintainer_pubkey,
//!     "my-repo",
//! );
//!
...
impl HotCache {
    ...
    fn add(&self, event: Event, pubkey: PublicKey, identifier: String, reason: RejectionReason) {
    ...
    fn get_maintainer_events(&self, pubkey: &PublicKey, identifier: &str) -> Vec<Event> {
...
impl ColdIndex {
    ...
    /// Invalidate (remove) maintainer announcements from cold index
    ///
    ...
    fn invalidate_maintainer_announcements(
        ...
        entries.retain(|_, entry| {
            // Keep entries that DON'T match the maintainer/identifier
            !(entry.pubkey == *maintainer_pubkey && entry.identifier == identifier)
        });
...
impl RejectedEventsIndex {
    ...
    /// Invalidate maintainer announcements and get events for immediate re-processing
    ///
    /// This is called when an owner announcement is accepted that lists a maintainer.
    ...
    pub fn invalidate_and_get_events(
        &self,
        maintainer_pubkey: &PublicKey,
        identifier: &str,
    ) -> (usize, Vec<Event>) {
...
mod tests {
    ...
    async fn test_hot_cache_stores_and_retrieves_events() {
        ...
        let retrieved = cache.get_maintainer_events(&pubkey, &identifier);
    ...
    async fn test_cold_index_invalidation() {
        ...
        let removed = index.invalidate_maintainer_announcements(&pubkey, &identifier);
        assert_eq!(removed, 1);
        assert!(!index.contains(&event.id));
    }
    ...
    async fn test_invalidate_and_get_events() {
        ...
        let (removed, hot_events) = index.invalidate_and_get_events(&pubkey, &identifier);
    ...
    async fn test_cleanup_expired_both_tiers() {
        ...
        let (hot_expired, cold_expired) = index.cleanup_expired();
