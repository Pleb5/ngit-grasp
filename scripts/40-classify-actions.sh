...
if [[ -f "$LOGS_DIR/purgatory-expired.txt" ]]; then
    while IFS=$'\t' read -r repo npub timestamp reason; do
...
log_info "Loaded ${#PURGATORY[@]} purgatory entries"
...
log_info "Loaded ${#PARSE_FAIL[@]} parse failure entries"
...
log_info "Loaded ${#DELETED[@]} deletion entries"
